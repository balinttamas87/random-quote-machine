...
(function () {
	...
	var quoteTemplate = 
		'<div class="inner-quote-wrapper" style="visibility: visible" id="inner-quote-wrapper">' +
			'<p id="quote" style="visibility: visible">{quote}</p>' +
			'<p id="author" style="visibility: visible">{author}</p>' +
		'</div>';
	...
	function showQuoteAndAuthor (theRandomQuote,theAuthorOfRandomQuote) {
		...
		quoteWrapper.innerHTML = quoteTemplate
			.replace('{quote}', theRandomQuote)
			.replace('{author}', theAuthorOfRandomQuote);
